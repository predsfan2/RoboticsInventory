...
const ACTION_LABELS = {
  ...
  KIT_ADD_COMPONENTS: '🧰 Kit Pieces Added',
  KIT_UPDATE_COMPONENT: '🧰 Kit Piece Updated',
  KIT_REMOVE_COMPONENT: '🧰 Kit Piece Removed',
  UNDO: '↩ Undo',
};
              ...
              <tbody>
                {logs.map((log, idx) => (
                  <tr key={log.id || idx} className={`${idx < logs.length - 1 ? 'border-b border-gray-800/50' : ''} hover:bg-gray-800/30`}>
                    ...
                    <td className="px-4 py-2.5"><span className="text-gray-300">{log.userName}</span></td>
                    <td className="px-4 py-2.5 text-xs">{ACTION_LABELS[log.action] || log.action}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-400 hidden md:table-cell max-w-[140px] truncate">{log.itemName || '—'}</td>
                    <td className="px-4 py-2.5 text-xs text-gray-500 hidden lg:table-cell max-w-[200px] truncate">{log.details || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function AuditTab() {
  const toast = useToast();
  const { user } = useAuth();
  const canUndo = hasPermission(user, 'admin.users');
  const [logs, setLogs] = useState([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);
  ...
  return (
    <>
      ...
      <p className="text-xs text-gray-600 mb-3">
        Audit entries with a before snapshot can be undone (item update, condition, move, stock adjust).
      </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-gray-500 border-b border-gray-800">
                  ...
                  <th className="text-left px-4 py-2 font-medium">Action</th>
                  <th className="text-left px-4 py-2 font-medium hidden md:table-cell">Item</th>
                  {canUndo && <th className="text-right px-4 py-2 font-medium">Undo</th>}
                </tr>
              </thead>
              <tbody>
                {logs.map((log, idx) => {
                  const undoable = canUndo && UNDOABLE.has(log.action) && !!log.before;
                  return (
                    <tr key={log.id || idx} className={`${idx < logs.length - 1 ? 'border-b border-gray-800/50' : ''} hover:bg-gray-800/30`}>
                      <td className="px-4 py-2.5 text-xs text-gray-500 whitespace-nowrap">
                        {new Date(log.timestamp || log.date).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </td>
                      <td className="px-4 py-2.5"><span className="text-gray-300">{log.userName}</span></td>
                      <td className="px-4 py-2.5 text-xs">{ACTION_LABELS[log.action] || log.action}</td>
                      <td className="px-4 py-2.5 text-xs text-gray-400 hidden md:table-cell max-w-[140px] truncate">{log.itemName || '—'}</td>
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex border-b border-gray-800 mb-4">
        <button onClick={() => setTab('activity')} className={`tab-btn ${tab === 'activity' ? 'tab-active' : 'tab-inactive'}`}>
          Activity
        </button>
        <button onClick={() => setTab('audit')} className={`tab-btn ${tab === 'audit' ? 'tab-active' : 'tab-inactive'}`}>
          Audit
        </button>
      </div>
