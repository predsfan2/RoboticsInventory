...
import {
  ...
  approveReimbursement, denyReimbursement,
} from '../../lib/api';
...
function SubmitModal({ onSave, onClose }) {
  ...
  const [form, setForm] = useState({ amount: '', reason: '', receiptUrl: '' });
      ...
      <div className="modal-panel max-w-sm p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div>
            ...
            <textarea className="input resize-none" rows={3} required value={form.reason} onChange={(e) => set('reason', e.target.value)} placeholder="Describe what you purchased and why…" />
          </div>
          <div>
            <label className="block text-xs text-gray-400 mb-1">Receipt URL</label>
            <input className="input" value={form.receiptUrl} onChange={(e) => set('receiptUrl', e.target.value)} placeholder="Google Drive, Dropbox link…" />
          </div>
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <button onClick={() => setAddOpen(true)} className="btn-primary">+ Request Reimbursement</button>
      </div>
        ...
        {['', 'pending', 'approved', 'denied'].map((s) => (
          <button key={s} onClick={() => setFilterStatus(s)}
            className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${
              filterStatus === s ? 'bg-indigo-600 border-indigo-500 text-white' : 'border-gray-700 text-gray-500 hover:border-gray-500'
            }`}>
            ...
            <div key={r.id} className="card p-4 flex flex-col sm:flex-row sm:items-center gap-3">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-3 text-xs text-gray-600 mt-1">
                  ...
                  {r.approvedAt && (
                    <span>{r.status === 'approved' ? '✓ Approved' : '✕ Denied'} {new Date(r.approvedAt).toLocaleDateString()} by {r.approvedBy}</span>
                  )}
                  {r.denialReason && <span>Reason: {r.denialReason}</span>}
                  {r.receiptUrl && (
                    <a href={r.receiptUrl} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">📎 Receipt</a>
                  )}
                </div>
              </div>
